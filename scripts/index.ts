...
let curWordIndex = 0;
const wordGuesses: Array<string> = Array(6).fill('');
let answer: string;
...
const getNewWord = () => {
	...
	xhr.onload = () => {
		if (xhr.status === 200) {
			const words: Array<string> = JSON.parse(xhr.responseText);
			answer = words[0].substring(0, 5);
			if (answer.length < 5) answer += 'e'.repeat(5 - answer.length);
		} else {
			answer = 'great';
		}
	}
...
(() => {
	...
	const clearBoardBtn = document.querySelector('#clearBoardBtn') as HTMLButtonElement;
